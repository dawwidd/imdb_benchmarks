...
docker-compose -f ten-instance-lru.yml up -d
...
REDIS_IP1=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(docker-compose -f ten-instance.yml ps -q redis0))
REDIS_IP2=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(docker-compose -f ten-instance.yml ps -q redis1))
REDIS_IP3=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(docker-compose -f ten-instance.yml ps -q redis2))
